...
%Phis
matrix(1,1) = quad(matlabFunction(phi_right(1)*phi_right(1)), startPoint, intervalLength+startPoint)+alpha*quad(matlabFunction(psi_right(1)*phi_right(1)),startPoint, intervalLength+startPoint);
...
matrix(2*(numIntervals),2*(numIntervals+1)-2)= quad(matlabFunction(phi_left(numIntervals+1)*phi_left(numIntervals+1)),(numIntervals-1)*intervalLength+startPoint, (numIntervals)*intervalLength+startPoint)+beta*quad(matlabFunction(psi_left(numIntervals+1)*phi_left(numIntervals+1)),(numIntervals-1)*intervalLength+startPoint, (numIntervals)*intervalLength+startPoint);
...
k=2;
%Phi(k) Phi(k-1)
matrix(2*k-3,2*k-2) = quad(matlabFunction(phi_left(k)*phi_right(k-1)),(k-2)*intervalLength+startPoint, (k-1)*intervalLength+startPoint);
...
%Corresponds to column Dk
%%Psi(k) Phi(k-1)
matrix(2*k-3,2*k-1) = quad(matlabFunction(psi_left(k)*phi_right(k-1)), (k-2)*intervalLength+startPoint, (k-1)*intervalLength+startPoint);
...
plot(Cpjs),title('pGuess (recompoosition)')
figure
...
%Dpjs(1,:)= linspace(startPoint,totalLength+startPoint,numIntervals+1);
for index=2:length(Cpjs)-1
   Cnjs(1,index) = Coeffs(2*index-2);
   Dnjs(1,index) = Coeffs(2*index-1);
end

Cnjs(1,1) = Coeffs(1);
Dnjs(1,1) = alpha*Coeffs(1);
Cnjs(1,numIntervals+1) = Coeffs(length(Coeffs));
Dnjs(1,numIntervals+1) = beta*Coeffs(length(Coeffs));

%Plotting
figure;
plot(y,subs(pGuess,x,y)),title('pGuess (original)')
%saveas(fig2,strcat('stability test/',num2str(numIntervals),'terms/pGuess.jpg'))

%figure;
%plot(y,subs(nGuess,x,y)),title('nGuess (original)')
%saveas(fig3,strcat('stability test/',num2str(numIntervals),'terms/nGuess.jpg'))

figure;
plot(guesstimate(1,:),guesstimate(2,:));title('all Ms');
%saveas(fig4,strcat('stability test/',num2str(numIntervals),'terms/output.jpg'))
