...
%Adjusts initial pGuess by linear scaling after calculating Gopp, Gopn to test stability
...
filepath=strcat('stability test v13/rand',num2str(randLow),'/',num2str(numIntervals),'terms');
mkdir(filepath)
...
pGuess = @(x) tweak*(p1*exp(-gammap1*x)+p2*exp(-gammap2*x)+p3*exp(-gammap3*x));
pprime = @(x) -tweak*(p1*gammap1*exp(-gammap1*x) + p2*gammap2*exp(-gammap2*x) + p3*gammap3*exp(-gammap3*x));
...
fig4 = figure;
plot(indexn,Cnjs,'x',indexn,nGuess(indexn));title('n(x) calculated'),legend('Cnjs','nGuess - original');
