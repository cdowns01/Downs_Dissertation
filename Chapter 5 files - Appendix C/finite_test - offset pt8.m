...
phi_right(index)= subs(f1,x,(x-(index-1)*intervalLength)/intervalLength);%*(heaviside((x-(index-1)*intervalLength)/intervalLength)-heaviside((x-(index)*intervalLength)/intervalLength));
phi_left(index)=subs(f2,x,(x-(index-2)*intervalLength)/intervalLength);%*(heaviside((x-(index-2)*intervalLength)/intervalLength)-heaviside((x-(index-1)*intervalLength)/intervalLength));

psi_right(index)=subs(f4,x,(x-(index-1)*intervalLength)/intervalLength);%*(heaviside((x-(index-1)*intervalLength)/intervalLength)-heaviside((x-(index)*intervalLength)/intervalLength));
psi_left(index)=subs(f3,x,(x-(index-2)*intervalLength)/intervalLength);%*(heaviside((x-(index-2)*intervalLength)/intervalLength)-heaviside((x-(index-1)*intervalLength)/intervalLength));
end
...
%Generate output basis functions.  These functions are the same as above,
%but multiplied by heaviside functions to remove all portions of the basis
%functions outside the valid integral
% for index=1:numIntervals+1
% phi_right(index)=subs(f1,x,(x-(index-1)*intervalLength)/intervalLength)*(heaviside((x-(index-1)*intervalLength)/intervalLength)-heaviside((x-(index)*intervalLength)/intervalLength));
% phi_left(index)=subs(f2,x,(x-(index-2)*intervalLength)/intervalLength)*(heaviside((x-(index-2)*intervalLength)/intervalLength)-heaviside((x-(index-1)*intervalLength)/intervalLength));
% 
...
% 
% solution = 0;
% for index = 2:numIntervals
% 
%     solution = solution + Coeffs(2*index-2)*phi_left(index) + Coeffs(2*index-1)*phi_right(index);
% 
%     solution = solution + Coeffs(2*index-1)*psi_left(index) + Coeffs(2*index-1)*psi_right(index);
% end
% 
% %Add first and last cases
% solution = solution + Coeffs(1)*phi_right(1) + Coeffs(1)*alpha*psi_right(1);
% 
% solution = solution + Coeffs(2*numIntervals)*phi_left(numIntervals+1) + Coeffs(2*numIntervals)*beta*psi_left(numIntervals+1)
% 
