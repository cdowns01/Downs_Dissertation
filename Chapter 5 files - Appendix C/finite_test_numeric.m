...
for k = 3:numIntervals-1
    ...
    %Phi(k) Phi(k-1)
     matrix1(2*k-4,2*k-2) = quad(matlabFunction(diff(phi_left(k),x,2)*phi_right(k-1)),(k-2)*intervalLength, (k-1)*intervalLength);

    %Phi(k) Psi(k-1)
     matrix1(2*k-3,2*k-2) = quad(matlabFunction(diff(phi_left(k),x,2)*psi_right(k-1)),(k-2)*intervalLength, (k-1)*intervalLength);
   
    %Phi(k) Phi(k)
    matrix1(2*k-2,2*k-2) = quad(matlabFunction(diff(phi_left(k),x,2)*phi_left(k)),(k-2)*intervalLength,(k-1)*intervalLength)+quad(matlabFunction(diff(phi_right(k),x,2)*phi_right(k)),(k-1)*intervalLength, (k)*intervalLength);
    
    %Phi(k) Psi(k)
    matrix1(2*k-1,2*k-2) = quad(matlabFunction(diff(phi_left(k),x,2)*psi_left(k)),(k-2)*intervalLength,(k-1)*intervalLength)+quad(matlabFunction(diff(phi_right(k),x,2)*psi_right(k)),(k-1)*intervalLength, (k)*intervalLength);
    
    %Phi(k) Phi(k+1)
    matrix1(2*k,2*k-2) = quad(matlabFunction(diff(phi_right(k),x,2)*phi_left(k+1)),(k-1)*intervalLength, (k)*intervalLength);
    
    %Phi(k) Psi(k+1)
    matrix1(2*k+1,2*k-2) = quad(matlabFunction(diff(phi_right(k),x,2)*psi_left(k+1)),(k-1)*intervalLength, (k)*intervalLength);   
     

    %%Calculate for interval's Psi basis function
    %Corresponds to column Dk
    %%Psi(k) Phi(k-1)
    matrix1(2*k-4,2*k-1) = quad(matlabFunction(diff(psi_left(k),x,2)*phi_right(k-1)), (k-2)*intervalLength, (k-1)*intervalLength);

    %Psi(k) Psi(k-1)
    matrix1(2*k-3,2*k-1) = quad(matlabFunction(diff(psi_left(k),x,2)*psi_right(k-1)), (k-2)*intervalLength, (k-1)*intervalLength);
   
    %Psi(k) Phi(k)
    matrix1(2*k-2,2*k-1) = quad(matlabFunction(diff(psi_left(k),x,2)*phi_left(k)),(k-2)*intervalLength, (k-1)*intervalLength)+quad(matlabFunction(diff(psi_right(k),x,2)*phi_right(k)),(k-1)*intervalLength, (k)*intervalLength);
    
    %Psi(k) Psi(k)
    matrix1(2*k-1,2*k-1) = quad(matlabFunction(diff(psi_left(k),x,2)*psi_left(k)),(k-2)*intervalLength, (k-1)*intervalLength)+quad(matlabFunction(diff(psi_right(k),x,2)*psi_right(k)),(k-1)*intervalLength,(k)*intervalLength);
    
    %Psi(k) Phi(k+1)
    matrix1(2*k,2*k-1) = quad(matlabFunction(diff(psi_right(k),x,2)*phi_left(k+1)),(k-1)*intervalLength, (k)*intervalLength);
    
    %Psi(k) Psi(k+1)
    matrix1(2*k+1,2*k-1) = quad(matlabFunction(diff(psi_right(k),x,2)*psi_left(k+1)),(k-1)*intervalLength, (k)*intervalLength);
...
matrix1(1,1) = quad(matlabFunction(diff(phi_right(1),x,2)*phi_right(1)), 0, intervalLength)+alpha*quad(matlabFunction(diff(psi_right(1),x,2)*phi_right(1)), 0, intervalLength);
...
matrix1(2,1) = quad(matlabFunction(diff(phi_right(1),x,2)*phi_left(2)),0, intervalLength)+alpha*quad(matlabFunction(diff(psi_right(1),x,2)*phi_left(2)),0, intervalLength);
matrix1(3,1) = quad(matlabFunction(diff(phi_right(1),x,2)*psi_left(2)),0, intervalLength)+alpha*quad(matlabFunction(diff(psi_right(1),x,2)*psi_left(2)),0, intervalLength);
...
matrix1(2*(numIntervals)-2,2*(numIntervals+1)-2)= quad(matlabFunction(diff(phi_left(numIntervals+1),x,2)*phi_right(numIntervals)),(numIntervals-1)*intervalLength, (numIntervals)*intervalLength)+beta*quad(matlabFunction(diff(psi_left(numIntervals+1),x,2)*phi_right(numIntervals)),(numIntervals-1)*intervalLength, (numIntervals)*intervalLength);
matrix1(2*(numIntervals)-1,2*(numIntervals+1)-2)= quad(matlabFunction(diff(phi_left(numIntervals+1),x,2)*psi_right(numIntervals)),(numIntervals-1)*intervalLength, (numIntervals)*intervalLength)+beta*quad(matlabFunction(diff(psi_left(numIntervals+1),x,2)*psi_right(numIntervals)),(numIntervals-1)*intervalLength, (numIntervals)*intervalLength);
matrix1(2*(numIntervals),2*(numIntervals+1)-2)= quad(matlabFunction(diff(phi_left(numIntervals+1),x,2)*phi_left(numIntervals+1)),(numIntervals-1)*intervalLength, (numIntervals)*intervalLength)+beta*quad(matlabFunction(diff(psi_left(numIntervals+1),x,2)*phi_left(numIntervals+1)),(numIntervals-1)*intervalLength, (numIntervals)*intervalLength);
...
k=2;
%Phi(k) Phi(k-1)
matrix1(2*k-3,2*k-2) = quad(matlabFunction(diff(phi_left(k),x,2)*phi_right(k-1)),(k-2)*intervalLength, (k-1)*intervalLength);
...
%Corresponds to column Dk
%%Psi(k) Phi(k-1)
matrix1(2*k-3,2*k-1) = quad(matlabFunction(diff(psi_left(k),x,2)*phi_right(k-1)), (k-2)*intervalLength, (k-1)*intervalLength);
...
for k = 2:numIntervals
    %Phi test function
    rhs_matrix(2*k-2) = quad(matlabFunction(RHS*phi_left(k)),(k-2)*intervalLength,(k-1)*intervalLength)+quad(matlabFunction(RHS*phi_right(k)),(k-1)*intervalLength,(k)*intervalLength);
    
    %Psi test function
    rhs_matrix(2*k-1) = quad(matlabFunction(RHS*psi_left(k)),(k-2)*intervalLength,(k-1)*intervalLength)+quad(matlabFunction(RHS*psi_right(k)),(k-1)*intervalLength,(k)*intervalLength);
...
%Handle 1st interval 'half case'
%Phi test function
rhs_matrix(1) = quad(matlabFunction(RHS*phi_right(1)),0,intervalLength);
%Psi test function
...
%Handle last interval 'half case'
%Phi test function
rhs_matrix(2*(numIntervals+1)-2) = quad(matlabFunction(RHS*phi_left(numIntervals+1)),(numIntervals-1)*intervalLength,(numIntervals)*intervalLength);
