...
for index=1:1:max1/decimation-min1/decimation
    for index2 = 1:1:max2/decimation-min2/decimation
        for index3 = 1:1:max3/decimation-min3/decimation
               efficiencies(index,index2,index3) = solar_efficiency3(index*decimation+min1,index2*decimation+min2,index3*decimation+min3,ref3,max_power);
        end
    end
    index
end
...
xout = xmax*decimation;  %Most recent value:1100
x_wave = xout+min1
yout = ymax*decimation;  %Most recent value:1344 
y_wave = yout+min2
zout = zmax*decimation; %Most recent value:1760
z_wave = zout+min3

max_efficiency  %Most recent value:12.06%
