...
%Modified Newton, noisy input, Jacobian, terms one and two only
...
filepath=strcat('modifiedNewton v5/rand',num2str(rand),'/',num2str(numIntervals),'terms');
mkdir(filepath)
...
[output4b,Jacobian4b,RHSOut,Rn1,Rn2,Rn3,Rp1,Rp2,Rp3]=callFunction4b(Coeffs);

options = optimoptions('fsolve','Display','iter','TolFun',10^10,'Jacobian','on','TypicalX',Coeffs);
[x,fval,exitflag,output,Jacobian]= fsolve(@(x)callFunction4(x),Coeffs,options);
