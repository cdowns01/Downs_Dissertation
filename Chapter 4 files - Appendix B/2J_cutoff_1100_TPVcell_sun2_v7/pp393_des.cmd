...
File {
	Grid = "n91_msh.tdr"
	Plot    = "n393_des.tdr"	*Commented out to limit memory usage on system.  Add back in for individual experiments, not long optimization simulations.
 	Current = "n393_des.plt"
	Output  = "n393_des.log"
	Parameter = "pp393_des.par"
	IlluminationSpectrum = "n137_am15d.txt"
      }
...
Solve   {
	 ...
	 * Plot (FilePrefix = "n393_sc")
    	
    	*CurrentPlot()
     	*System("rm -f n393_banddgm_des.tdr")
     	
     	Quasistationary ( 
   		 InitialStep=5e-2 MaxStep =1e-2 MinStep = 1e-3 
    		Goal{ voltage = .8 Name = "front" }
 	   ){ Coupled {Poisson Electron Hole  } } 
    
  	 Quasistationary ( 
   		 InitialStep=5e-2 MaxStep =1e-3 MinStep = 1e-3 
    		Goal{ voltage = 2.7 Name = "front" }
 	   ){ Coupled {Poisson Electron Hole  } }  
            }
