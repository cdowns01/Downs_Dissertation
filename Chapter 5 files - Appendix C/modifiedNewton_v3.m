...
%Modified Newton, no noise, Jacobian
...
filepath=strcat('modifiedNewton v3/rand',num2str(rand),'/',num2str(numIntervals),'terms');
mkdir(filepath)
...
% n3rand=1.07209*10^12;
% n4rand=-9.73843*10^12;
% gamman1rand=100000*rand;

nGuess = nGuessOrig;%@(x) (n1rand*exp(-gamman1rand*x)+n2rand*exp(-gamman2*x)+n3rand*exp(-gamman3*x)+n4rand*exp(-gamman4*x));
nprime = nprimeOrig;%@(x) -(n1rand*gamman1rand*exp(-gamman1rand*x) + n2rand*gamman2*exp(-gamman2*x) + n3rand*gamman3*exp(-gamman3*x)+n4rand*gamman4*exp(-gamman4*x));
nprimeprime = nprimeprimeOrig;%@(x) n1rand*gamman1rand*gamman1rand*exp(-gamman1rand*x) + n2rand*gamman2*gamman2*exp(-gamman2*x) + n3rand*gamman3*gamman3*exp(-gamman3*x)+n4rand*gamman4*gamman4*exp(-gamman4*x);
...
pGuess = pGuessOrig;%@(x) (p1rand*exp(-gammap1rand*x)+p2rand*exp(-gammap2*x)+p3rand*exp(-gammap3*x)+p4rand*exp(-gammap4*x));
pprime = pprimeOrig;%@(x) -(p1rand*gammap1rand*exp(-gammap1rand*x) + p2rand*gammap2*exp(-gammap2*x) + p3rand*gammap3*exp(-gammap3*x)+p4rand*gammap4*exp(-gammap4*x));
pprimeprime =pprimeprimeOrig;%@(x) p1rand*gammap1rand*gammap1rand*exp(-gammap1rand*x) + p2rand*gammap2*gammap2*exp(-gammap2*x) + p3rand*gammap3*gammap3*exp(-gammap3*x)+p4rand*gammap4*gammap4*exp(-gammap4*x);
...
[output3,Jacobian3,RHSOut,Rn1,Rn2,Rn3,Rp1,Rp2,Rp3]=callFunction3(Coeffs);

options = optimoptions('fsolve','Display','iter','TolFun',10^15,'Jacobian','on','TypicalX',Coeffs);%
[x,fval,exitflag,output,jacobian]= fsolve(@(x)callFunction2(x),Coeffs,options);
