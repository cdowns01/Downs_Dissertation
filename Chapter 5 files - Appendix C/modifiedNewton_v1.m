...
%Modified Newton, no noise, no Jacobian
...
filepath=strcat('modifiedNewton v1/rand',num2str(rand),'/',num2str(numIntervals),'terms');
mkdir(filepath)
...
% n3rand=10^12*(rand);
% 
nGuess =  nGuessOrig;%@(x)(n1rand*exp(-gamman1*x)+n2rand*exp(-gamman2*x)+n3rand*exp(-gamman3*x));
nprime =  nprimeOrig;%@(x)-(n1rand*gamman1*exp(-gamman1*x) + n2rand*gamman2*exp(-gamman2*x) + n3rand*gamman3*exp(-gamman3*x));
nprimeprime = nprimeprimeOrig;%@(x)n1rand*gamman1*gamman1*exp(-gamman1*x) + n2rand*gamman2*gamman2*exp(-gamman2*x) + n3rand*gamman3*gamman3*exp(-gamman3*x);
...
% p3rand=-8*10^13*rand;
% 
pGuess = pGuessOrig;%@(x) (p1rand*exp(-gammap1*x)+p2rand*exp(-gammap2*x)+p3rand*exp(-gammap3*x));
pprime =  pprimeOrig;%@(x)-(p1rand*gammap1*exp(-gammap1*x) + p2rand*gammap2*exp(-gammap2*x) + p3rand*gammap3*exp(-gammap3*x));
pprimeprime = pprimeprimeOrig;%@(x)p1rand*gammap1*gammap1*exp(-gammap1*x) + p2rand*gammap2*gammap2*exp(-gammap2*x) + p3rand*gammap3*gammap3*exp(-gammap3*x);
...
for index = 3:4:length(D)-2
   D(index,index) = 1;
   D(index+1,index+1) = 1;%10^4*numIntervals
   D(index+2,index+2) = 1;
   D(index+3,index+3) = 1;%10^4*numIntervals
end
D(1,1)=1;
D(2,2)=1;
...
options = optimoptions('fsolve','MaxIter',maxIterations,'Display','iter','TolFun',10^6);
[x,fval,exitflag,output,jacobian]= fsolve(@(x)callFunction(x),Coeffs,options);
count=2
