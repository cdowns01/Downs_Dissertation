...
filepath=strcat('newton v3/rand',num2str(randLow),'/',num2str(numIntervals),'terms');
mkdir(filepath)
...
Gopn = @(x) cn1.*(n1*(gamman1*gamman1)*exp(-gamman1*x) + n2*(gamman2*gamman2)*exp(-gamman2*x) + n3*(gamman3*gamman3)*exp(-gamman3*x))    +(ksi*T*mun/q*(pprimeprime(x)./pGuess(x) - (pprime(x)./pGuess(x)).^2) - 1/taun).*(n1*exp(-gamman1*x) + n2*exp(-gamman2*x) + n3*exp(-gamman3*x))+cn2.*pprime(x)./pGuess(x).*(-n1*(gamman1)*exp(-gamman1*x) - n2*(gamman2)*exp(-gamman2*x) - n3*(gamman3)*exp(-gamman3*x))     
Gopp = @(x) cp1.*(p1*(gammap1*gammap1)*exp(-gammap1*x) + p2*(gammap2*gammap2)*exp(-gammap2*x) + p3*(gammap3*gammap3)*exp(-gammap3*x))    +(ksi*T*mup/q*((nprime(x)./nGuess(x)).^2 - nprimeprime(x)./nGuess(x)) - 1/taup).*(p1*exp(-gammap1*x) + p2*exp(-gammap2*x) + p3*exp(-gammap3*x))+cp2.*nprime(x)./nGuess(x).*(-p1*(gammap1)*exp(-gammap1*x) - p2*(gammap2)*exp(-gammap2*x) - p3*(gammap3)*exp(-gammap3*x))     
...
for k = 1:numIntervals-2
   k=k+1;
    ...
    pBlock = @(x) Cp3*(Cnjs(k).*f1primeprime(x-node(k)) + Cnjs(k+1).*f2primeprime(x-node(k)) + Dnjs(k).*f3primeprime(x-node(k)) + Dnjs(k+1).*f4primeprime(x-node(k))).*(Cpjs(k).*f1(x-node(k)) + Cpjs(k+1).*f2(x-node(k)) + Dpjs(k).*f3(x-node(k)) + Dpjs(k+1).*f4(x-node(k)))./(Cnjs(k).*f1(x-node(k)) + Cnjs(k+1).*f2(x-node(k)) + Dnjs(k).*f3(x-node(k)) + Dnjs(k+1).*f4(x-node(k))).^2 - Cp3*(Cnjs(k).*f1prime(x-node(k)) + Cnjs(k+1).*f2prime(x-node(k)) + Dnjs(k).*f3prime(x-node(k)) + Dnjs(k+1).*f4prime(x-node(k))).^2.*(Cpjs(k).*f1(x-node(k)) + Cpjs(k+1).*f2(x-node(k)) + Dpjs(k).*f3(x-node(k)) + Dpjs(k+1).*f4(x-node(k)))./(Cnjs(k).*f1(x-node(k)) + Cnjs(k+1).*f2(x-node(k)) + Dnjs(k).*f3(x-node(k)) + Dnjs(k+1).*f4(x-node(k))).^3;%- Cp2*(Cnjs(k).*f1prime(x-node(k)) + Cnjs(k+1).*f2prime(x-node(k)) + Dnjs(k).*f3prime(x-node(k)) + Dnjs(k+1).*f4prime(x-node(k))).*(Cpjs(k).*f1prime(x-node(k)) + Cpjs(k+1).*f2prime(x-node(k)) + Dpjs(k).*f3prime(x-node(k)) + Dpjs(k+1).*f4prime(x-node(k)))./(Cnjs(k).*f1(x-node(k)) + Cnjs(k+1).*f2(x-node(k)) + Dnjs(k).*f3(x-node(k)) + Dnjs(k+1).*f4(x-node(k))).^2;
    k=k-1;
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %Block 1  - (Upper left 4x4)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
fig4 = figure;
plot(indexn,Cnjs,'x',indexn,nOrig(indexn));set(gca,'FontSize',14),title('n(x) calculated'),legend('Cnjs','nGuess - original'),xlabel('Depth (in cm)'),ylabel('Carrier concentration (in cm^-3)');
saveas(fig4,strcat(filepath,'/n_calculated_iteration',num2str(iterationCount),'.jpg'))

fig6 = figure;
plot(indexn,Dnjs,'x',indexn,nprimeOrig(indexn)),set(gca,'FontSize',14),title('nprime calculated'),legend('Dnjs','nprime - original'),xlabel('Depth (in cm)'),ylabel('Carrier concentration (in cm^-3)');
...
fig4 = figure;
plot(indexn,Cpjs,'x',indexn,pOrig(indexn));set(gca,'FontSize',14),title('p(x) calculated'),legend('Cpjs','pGuess - orignial'),xlabel('Depth (in cm)'),ylabel('Carrier concentration (in cm^-3)');
saveas(fig4,strcat(filepath,'/p_calculated_iteration',num2str(iterationCount),'.jpg'))

fig6 = figure;
plot(indexn,Dpjs,'x',indexn,pprimeOrig(indexn)),set(gca,'FontSize',14),title('pprime calculated'),legend('Dpjs','pGuess - orignial'),xlabel('Depth (in cm)'),ylabel('Carrier concentration (in cm^-3)');
