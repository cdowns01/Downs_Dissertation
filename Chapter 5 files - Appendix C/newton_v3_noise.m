...
ksi = 0*1.38*10^-23; %(*boltzmann constant in J/K*)
ksiN = 1.38*10^-23;
ksiP = 0*1.38*10^-23;
...
filepath=strcat('newton v3/rand',num2str(rand),'/',num2str(numIntervals),'terms');
mkdir(filepath)
...
Gopn = @(x) cn1.*(n1*(gamman1*gamman1)*exp(-gamman1*x) + n2*(gamman2*gamman2)*exp(-gamman2*x) + n3*(gamman3*gamman3)*exp(-gamman3*x))    +(ksiN*T*mun/q*(pprimeprime(x)./pGuess(x) - (pprime(x)./pGuess(x)).^2) - 1/taun).*(n1*exp(-gamman1*x) + n2*exp(-gamman2*x) + n3*exp(-gamman3*x));%+cn2.*pprime(x)./pGuess(x).*(-n1*(gamman1)*exp(-gamman1*x) - n2*(gamman2)*exp(-gamman2*x) - n3*(gamman3)*exp(-gamman3*x))     
Gopp = @(x) cp1.*(p1*(gammap1*gammap1)*exp(-gammap1*x) + p2*(gammap2*gammap2)*exp(-gammap2*x) + p3*(gammap3*gammap3)*exp(-gammap3*x))    +(ksiP*T*mup/q*((nprime(x)./nGuess(x)).^2 - nprimeprime(x)./nGuess(x)) - 1/taup).*(p1*exp(-gammap1*x) + p2*exp(-gammap2*x) + p3*exp(-gammap3*x));%+cp2.*nprime(x)./nGuess(x).*(-p1*(gammap1)*exp(-gammap1*x) - p2*(gammap2)*exp(-gammap2*x) - p3*(gammap3)*exp(-gammap3*x))     

%Adjust pGuess to include error
%Adjust pGuess to include error
nOrig = nGuess;
nprimeOrig = nprime;
nprimeprimeOrig = nprimeprime;
...
pOrig = pGuess;
pprimeOrig = pprime;
pprimeprimeOrig = pprimeprime;

%p3*rand,gammap1,gammap2,gammap3 set;p1 and p2 calculated from those
%numbers using BCs
%Calcs done in Mathematica

p1rand=-2.32455*10^11;
p2rand=1.27855*10^12;
p3rand=-10^8*rand;
...
temp2 = ksiN*T*mun/q*(pprimeprimeOrig(node)./pOrig(node) - (pprimeOrig(node)./pOrig(node)).^2)-1/taun;
...
testCnjs2(iterationCount)=delCnjs(2);
testCpjs(iterationCount)=delCpjs(1);
testCpjs2(iterationCount)=delCpjs(2);
fig4 = figure;
