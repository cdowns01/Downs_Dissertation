...
%Modified Newton, noisy input, Jacobian
...
filepath=strcat('modifiedNewton v4/rand',num2str(rand),'/',num2str(numIntervals),'terms');
mkdir(filepath)
...
fig4 = figure;
plot(indexn,Cnjs,'x',node,nGuess(node),indexn,nOrig(indexn)),title('n(x) calculated'),legend('n(x) - calculated','n(x) - input','n(x) - original');set(gca,'FontSize',16),xlabel('Depth (in cm)'),ylabel('Carrier concentration (in cm^-3)');
...
fig5 = figure;
plot(indexn,Cpjs,'x',node,pGuess(node),indexn,pOrig(indexn)),title('p(x) calculated'),legend('p(x) - calculated','p(x) - input','p(x) - orignial');set(gca,'FontSize',16),xlabel('Depth (in cm)'),ylabel('Carrier concentration (in cm^-3)');
...
fig6=figure;
plot(indexn,(nGuess(node)-Cnjs).^2)
saveas(fig6,strcat(filepath,'/Recomposition difference n(x).jpg'))

fig6=figure;
plot(indexn,(pGuess(node)-Cpjs).^2)
saveas(fig6,strcat(filepath,'/Recomposition difference p(x).jpg'))


[output3,Jacobian3,RHSOut,Rn1,Rn2,Rn3,Rp1,Rp2,Rp3]=callFunction3(Coeffs);

options = optimoptions('fsolve','Display','iter','TolFun',10^15,'Jacobian','on','TypicalX',Coeffs,'MaxIter',maxIterations);%
