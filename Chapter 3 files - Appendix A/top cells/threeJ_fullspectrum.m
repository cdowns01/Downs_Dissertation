...
total_incidence = 0;
for index = 1:a-1
    if (ref3(index,1)<1318)
        total_incidence = total_incidence + ref3(index,2);
    end
end
...
for index=min/decimation:1:max1/decimation
    for index2 = index:1:max2/decimation
        for index3 = index2:1:max3/decimation
                efficiencies(index,index2,index3) = solar_efficiency3(index*decimation,index2*decimation,index3*decimation,ref3,max_power);
        end
    end
    index
end
...
xout = xmax*decimation  %Most recent value:654
yout = ymax*decimation  %Most recent value:910
zout = zmax*decimation  %Most recent value:1318
