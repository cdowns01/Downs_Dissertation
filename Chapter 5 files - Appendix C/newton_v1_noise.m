...
filepath=strcat('newton v1/rand',num2str(rand),'/',num2str(numIntervals),'terms');
mkdir(filepath)
...
weight=1;
Cnjs = Cnjs+delCnjs*weight;
Cpjs = Cpjs+delCpjs*weight;
Dnjs = Dnjs+delDnjs*weight;
Dpjs = Dpjs+delDpjs*weight;

testCnjs(iterationCount)=delCnjs(1);
fig4 = figure;
plot(indexn,Cnjs,'x',indexn,nGuess(indexn),indexn,nOrig(indexn),indexn,InitialCnjs,'x');title('n(x) calculated'),legend('Cnjs','nGuess - error','nGuess - original','InitialCnjs');
